...
REDIS_NODES = os.getenv("REDIS_NODES", None)


class ClusterRedisWrapper:
    def __init__(self, client: RedisCluster):
        self._client = client

    def pipeline(self, *args, **kwargs):
        # Ignore any explicit transaction flag and always use a
        # non-transactional pipeline, which is supported in cluster mode.
        kwargs.pop("transaction", None)
        return self._client.pipeline(transaction=False, *args, **kwargs)
...
async def startup():
    ...
    Any exceptions here are printed to stderr so they are visible
    in Docker logs even if logging is misconfigured.
    """
    try:
        if REDIS_NODES:
            nodes = []
            for item in os.getenv("REDIS_NODES").split(","):
                host, port = item.split(":")
                nodes.append(ClusterNode(host=host, port=int(port)))
            redis_cluster = RedisCluster(startup_nodes=nodes, decode_responses=True)
            redis = ClusterRedisWrapper(redis_cluster)
            FastAPICache.init(RedisBackend(redis), prefix="api:cache")
    except Exception:
        print("Fatal error during FastAPI startup:", file=sys.stderr)
        traceback.print_exc()
        raise
